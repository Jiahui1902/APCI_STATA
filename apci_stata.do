...
****************************************************
// define xi3 to do address other coding schemes
****************************************************
...
program define xyz
	//syntax varlist, age(varname) period(varname)
...
local names: colnames coefficients
...
local nm2 "Period 1"
forvalues i= 2/`p'{
local nm2 "`nm2'" "Period `i'"
}

mata: a=`a'
mata: p=`p'

mata: coef_full=J(1,0,.)
quietly ds _I*
local aaaa = "`r(varlist)'"
foreach i in `aaaa'{
matrix aaa=coefficients[1,"`i'"]
mata: coef_full=(coef_full,st_matrix("aaa"))
}

mata: stderr_sub=J(1,0,.)
mata: stderr_full=J(0,a-1+p-1+(a-1)*(p-1),.)
quietly ds _I*
local aaaa = "`r(varlist)'"
foreach i in `aaaa'{
matrix bbb=stderr["`i'",.]
...
foreach j in `aaaa'{
matrix stdelement=bbb[1,"`j'"]
mata: stderr_sub=(stderr_sub,st_matrix("stdelement"))
}
mata: stderr_full=(stderr_full\stderr_sub)
mata: stderr_sub=J(1,0,.)
}


//matrix coef_full=coefficients[1..1,1..`a'-1+`p'-1+(`a'-1)*(`p'-1)]
//mata: coef_full=st_matrix("coef_full")

//matrix stderr_full=stderr[1..`a'-1+`p'-1+(`a'-1)*(`p'-1),1..`a'-1+`p'-1+(`a'-1)*(`p'-1)]
//mata: stderr_full=st_matrix("stderr_full")

mata: apci_full = J(a+p+a*p,a-1+p-1+(a-1)*(p-1),0)
mata:
apci_full[1,1..(a-1)]=J(1,(a-1),-1)
apci_full[1+a,a..(a-1+p-1)]=J(1,(p-1),-1)
for (i=2; i<=a; i++) {
apci_full[i,i-1] = 1
}
for (i=2; i<=p; i++) {
apci_full[i+a,a-1+i-1] = 1
}
`end_mata'
...
for (c=a;c<=a-1+p-1;c++){
for (r=1; r<=a; r++) {
xxx=(xxx\ apci_full[r,i]*apci_full[(a+1..a+p),c])
}
yyy=(yyy,xxx)
xxx=J(0,1,.)
}
...
apci_full[(a+p+1)..(a+p+a*p),(a+p-1)..(a-1 + p-1 + (a-1)*(p-1))]=yyy
 `end_mata'

mata: ests = apci_full*coef_full'
mata: ests_se = apci_full*stderr_full'*apci_full'
***************************************************************
******* age's effect ***********
***************************************************************
mata:
...
mata: st_matrix("age", main)
mat rown age = "`nm1'"
mat coln age = "estimates" "Std Err" "P>|t|"
mat list age
***************************************************************
******* period's effect ***********
***************************************************************
...
main=J(0,3,.)
for (i=1;i<=p;i++){
main=(main\(ests[i+a,1],sqrt(diagonal(ests_se))[i+a,1] , 2 * ttail(df_r, abs(ests[i+a,1]/sqrt(diagonal(ests_se))[i+a,1])))  )
}
 `end_mata'
mata: st_matrix("period", main)
mat rown period = "`nm2'"
mat coln period = "estimates" "Std Err" "P>|t|"
mat list period
***************************************************************
******* cohort effect : inter***********
***************************************************************
**** cohort average effect ****
mata:
index=J(a,p,0)
cohort_average=J(a+p-1,a*p,0)
for (r=1;r<=a;r++){
for (c=1;c<=p;c++){
index[r,c] = r+p-c
for (k=1;k<=a+p-1;k++){
if(r+p-c==k){
// from young to old
cohort_average[a+p-k,p*(r-1)+c]=1
}
}
}
}
for (k=1;k<=a+p-1;k++){
cohort_average[k,]=cohort_average[k,]/sum(cohort_average[k,])
}
 `end_mata'
 
mata: ests_avg = cohort_average*ests[(a+p)+1..(a+p+a*p),]
mata: ests_avg_se = sqrt(diagonal(cohort_average*ests_se[(a+p)+1..(a+p+a*p),(a+p)+1..(a+p+a*p)]'*cohort_average'))

mata:
ests_p=J(0,1,.)
for(i=1;i<=a+p-1;i++){
ests_p=(ests_p \ 2 * ttail(df_r, abs(ests_avg[i,1]/ests_avg_se[i,1])))
}
ests_average = (ests_avg, ests_avg_se, ests_p)
`end_mata'
mata: st_matrix("cohort_average", ests_average)

local n=`a'+`p'-1
local nm_i "Cohort 1"
forvalues i= 2/`n'{
local nm_i "`nm_i'" "Cohort `i'"
}

mat rown cohort_average = "`nm_i'"
mat coln cohort_average = "estimates" "Std Err" "P>|t|"
mat list cohort_average
...
for (k=2;k<=a+p-2;k++){
num1=(num1, sum(cohort_average[1..k-1,]))
num2=(num2, sum(cohort_average[1..k-1,])+sum(cohort_average[k,])-1)
}
cohort_average[1,]=J(1,a*p,.)
cohort_average[a+p-1,]=J(1,a*p,.)
//num1
//num2
`end_mata'
...
mata yyy=J(1,0,.)
forvalues i= 1/`n'{
mata: st_matrix("value",sum(cohort_average[`i',]))
local v=value[1,1]
if (`v'>1){
forvalues l=1/`v'{
rcall: {con = contr.poly(`v')[`l',1]}
local xxx=`r(con)'
mata yyy = (yyy,`xxx')
}
mata cohort_average[`i',location[1,num1[1,`i'-1]..num2[1,`i'-1]]]=yyy
mata yyy=J(1,0,.)
}
}


mata: ests_intra_avg = cohort_average*ests[(a+p)+1..(a+p+a*p),]
mata: ests_intra_avg_se = sqrt(diagonal(cohort_average*ests_se[(a+p)+1..(a+p+a*p),(a+p)+1..(a+p+a*p)]'*cohort_average'))


mata:
ests_intra_p=J(0,1,.)
for(i=1;i<=a+p-1;i++){
ests_intra_p=(ests_intra_p \ 2 * ttail(df_r, abs(ests_intra_avg[i,1]/ests_intra_avg_se[i,1])))
}
ests_intra_average = (ests_intra_avg, ests_intra_avg_se, ests_intra_p)
`end_mata'
mata: st_matrix("intra_cohort", ests_intra_average)

mat rown intra_cohort = "`nm_i'"
mat coln intra_cohort = "estimates" "Std Err" "P>|t|"
mat list intra_cohort
